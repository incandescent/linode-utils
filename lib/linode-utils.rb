...
module LinodeUtils
  ...
  module LinodeOperations
    ...
    def create_config(api, linode_id, kernel_label, root_image_id)
      ...
      swap_disks = LinodeUtils.normalize_array(get_disks(api, linode_id).select(&CRITERIA_SWAP))
      
      raise "No Swap image found" if swap_disks.empty?
      
      swap_id = swap_disks[0].diskid

      LOG.info "Using root disk #{root_image_id} and swap #{swap_id}"
